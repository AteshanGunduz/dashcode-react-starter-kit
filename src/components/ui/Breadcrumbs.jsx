...
const Breadcrumbs = ({toggleLanguage}) => {
        ...
        <div className="md:mb-6 mb-4 flex space-x-3 rtl:space-x-reverse">
          <ul className="breadcrumbs">
            ...
            {groupTitle && (
              <li className="text-primary-500">
                <button type="button" className="capitalize">
                  {!toggleLanguage ? groupTitle : "Çoklu Seviye"}
                </button>
                <span className="breadcrumbs-icon rtl:transform rtl:rotate-180">
                  <Icon icon="heroicons:chevron-right" />
                </span>
            ...
            <li className="capitalize text-slate-500 dark:text-slate-400">
              {locationName === "dashboard" ? (<div>{ !toggleLanguage ? locationName : `Gösterge Paneli`}</div>):(<div>{ !toggleLanguage ? locationName : `Seviye ${locationName.slice(5)}`}</div>)}

              {/* { !toggleLanguage ? locationName : `Seviye ${locationName.slice(5)}`} */}
